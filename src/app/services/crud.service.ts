...
export class CrudService {
  ...
  createUser(user: User): Observable<void> {
    const url: string = environment.apiUrl + this.userUrl + this.createUrl;
    return this.httpClient.post<void>(url, user);
  }

  viewUser(userId: number): Observable<User> {
    const url: string = environment.apiUrl + this.userUrl + '/' + userId;
  ...
  listUsers(page: number): Observable<UserPage> {
    let url: string = environment.apiUrl + this.userUrl + this.listUrl;
    if(page !== 0) {
      url += '?page=' + page;
    }
  ...
  editUser(userId: number, newUser: User): Observable<void> {
    const url: string = environment.apiUrl + this.userUrl + this.editUrl + '/' + userId;
  ...
  deleteUser(user: User): Observable<void> {
    const url: string = environment.apiUrl + this.userUrl + this.deleteUrl + '/' + user.userId;
